...
| 要点 | 做法 |
|------|------|
| 数据来源 | **只用 TelecomSeed**（T2/T3 公开来源，200–500 张）；🔒 TelecomEval 测试集绝不可入微调集 |
...
> **为什么这条路线最值得投入**：它保留了真实图像的背景、光照、噪声分布，域间隙最小，同时精确产生了最难获取的**违规样本**。
>
> **v2.0 补充**：在取消现场采集后，本项目真实图像总量仅 200–500 张，Inpainting 路线「以少量真实图像派生大量违规样本」的能力变得更关键。建议把该路线的产出占比从 30% 上调至 **40–50%**，相应下调纯 T2I 占比。真实世界里工人违规的照片本就稀少且难以合法拍摄——生成式方案在这里既解决数据问题又规避了伦理问题，这个论点在论文里非常有力。
